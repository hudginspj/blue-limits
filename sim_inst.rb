    ...
    def read(count_100hz, time)
      ...
      pending_packets.each do |packet|
        ...
        when 'HEALTH_STATUS'
		  ...
		  #cycle_tlm_item(packet, 'temp1', -50.0, 50.0, 5.0)
		  #packet.write('temp1', 5.0)
		  ...
		  #while
		   ...
		   #message.tr('[]',' ').to_f
		   #packet.write('BATTERY', message.tr('[]',' ').to_f)
		   variables = message.tr('[]',' ').split(",").map { |s| s.to_f }
		   puts "mode"
		   puts variables[0]
		   packet.write('MODE', variables[0])
		   packet.write('TEMP2', variables[1])
		   packet.write('ACCELX', variables[2])
